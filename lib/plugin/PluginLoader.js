...
class PluginLoader {
  ...
  /**
   * Check if a plugin manifest is compatible
   * with the current OS according to npm's
   * package.json os field semantics
   *
   * @example
   * ["!darwin"] > Plugin is not compatible with darwin
   * ["darwin", "!win32"] > Plugin is compatible with darwin but not win32
   *
   * @param { PluginManifest } manifest
   * @returns { boolean }
   */
  isCompatibleWithCurrentOS (manifest) {
    if (!Array.isArray(manifest.os) || manifest.os.length === 0) return true

    const currentPlatform = platform.getPlatform()

    const blockedPlatforms = manifest.os
      .filter(os => typeof os === 'string' && os.startsWith('!'))
      .map(os => os.slice(1))

    const allowedPlatforms = manifest.os
      .filter(os => typeof os === 'string' && !os.startsWith('!'))

    if (blockedPlatforms.includes(currentPlatform)) {
      return false
    }
  ...
  async load (absBundlePath) {
    ...
    if (!this.isCompatibleWithCurrentOS(manifest)) {
      logger.warn('Skipping plugin', manifest.name, `as it's not compatible with the current platform (${platform.getPlatform()})`)
      return
    }
